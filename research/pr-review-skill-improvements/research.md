...
- **Recommendation**: **A**, broken into stages so each can ship independently.
  ...
  - **Stage 2 (single-pass synthesizer with precision enforcement)**: Opus 4.7 synthesizer receives all critic outputs + full diff in one context, applies evidence-grounding check (finding must quote offending lines) AND rubric scoring in a single pass, emits labeled output + footer of `N considered, X surfaced, Y dropped`. No separate validator subagents.
  - **Stage 3 (renderer + pending-review posting)**: synthesizer emits structured JSON (path, line, start_line, side, body, suggestion); renderer converts to `gh api POST /pulls/{n}/reviews` payload with `event=PENDING` by default. `--submit` flag switches to `event=COMMENT/APPROVE/REQUEST_CHANGES`. Paste-ready markdown is the fallback when posting fails (auth, out-of-hunk, stale commit_id).
